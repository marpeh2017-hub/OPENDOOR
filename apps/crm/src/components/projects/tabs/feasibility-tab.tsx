...
import { useAddFeasibilityItem, useCreateFeasibility, useDeleteFeasibilityItem, useFeasibility, useUpdateFeasibilityItem, type ProfileInput } from '@/hooks/use-feasibility'
...
export function ProjectFeasibilityTab({ projectId }: { projectId: string }) {
  ...
  return <div className="space-y-5">
    <div className="card-surface p-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
      <div><h2 className="text-lg font-bold">דוח אפס · {PROJECT_TYPES.find(x => x[0] === data.projectType)?.[1] ?? data.projectType}</h2><p className="text-sm text-muted-foreground">תאריך קובע: {new Date(data.valuationDate).toLocaleDateString('he-IL')} · מצב: טיוטה</p></div>
      <div className="inline-flex items-center gap-2 text-xs text-amber-800 bg-amber-50 border border-amber-200 rounded-md px-3 py-2"><ShieldAlert size={15} />נתוני המקור טרם חושבו לרווחיות</div>
    </div>
    ...
    <FoundationSection title="מקורות" icon={<BookOpen size={17} />} count={data.sources.length} canEdit={canEdit} path="sources" projectId={projectId} documents={documents.data ?? []} fields={[['type','סוג מקור',true,'PLANNING'],['title','כותרת',true],['issuer','מנפיק'],['sourceDate','תאריך',false,'date'],['sourceUrl','כתובת מקור'],['pageReference','עמוד/סעיף']]}> 
...
function FoundationSection({ title, icon, count, canEdit, projectId, path, fields, sources = [], documents = [], children }: { title: string; icon: React.ReactNode; count: number; canEdit: boolean; projectId: string; path: ItemPath; fields: SimpleField[]; sources?: Array<{ id: string; title: string }>; documents?: DocumentMeta[]; children: React.ReactNode }) {
  ...
  return <section className="card-surface p-5"><div className="flex items-center justify-between gap-3"><h3 className="flex items-center gap-2 text-sm font-semibold">{icon}{title}<span className="rounded-full bg-muted px-2 py-0.5 text-xs text-muted-foreground">{count}</span></h3>{canEdit && <Button variant="outline" size="sm" onClick={() => setOpen(!open)}><Plus size={14} className="ml-1" />הוספה</Button>}</div>{count ? <ul className="mt-3 divide-y divide-border">{children}</ul> : <p className="mt-3 text-sm text-muted-foreground">עדיין לא הוזנו נתונים.</p>}{open && <form id={`feasibility-${path}`} onSubmit={submit} className="mt-4 grid grid-cols-1 gap-3 border-t pt-4 sm:grid-cols-2">{fields.map(([key,label,required,kind]) => <Field key={key} label={label}>{key === 'type' ? <select required={required} value={values[key] ?? kind ?? ''} onChange={e => set(key, e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="PLANNING">תכנון</option><option value="APPRAISAL">שמאות</option><option value="GOVERNMENT">ממשלתי</option><option value="USER">הזנה ידנית</option><option value="OTHER">אחר</option></select> : <Input required={required} type={kind === 'date' ? 'date' : 'text'} value={values[key] ?? (kind && key !== 'type' && key !== 'sourceDate' ? kind : '')} onChange={e => set(key, e.target.value)} />}</Field>)}{path !== 'sources' && <><Field label="מקור נתון"><select value={values.sourceId ?? ''} onChange={e => set('sourceId', e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="">ללא מקור מקושר — תופיע אזהרת איכות</option>{sources.map(source => <option key={source.id} value={source.id}>{source.title}</option>)}</select></Field><Field label="סיווג הנתון"><select value={values.classification} onChange={e => set('classification', e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="FACT">עובדה</option><option value="SOURCE_DATA">נתון מקור</option><option value="ASSUMPTION">הנחה</option><option value="ESTIMATE">אומדן</option><option value="USER_OVERRIDE">שינוי ידני מאושר</option></select></Field><Field label="רמת ודאות"><select value={values.confidence} onChange={e => set('confidence', e.target.value)} className="h-10 rounded-md border border-input bg-background px-3 text-sm"><option value="HIGH">גבוהה</option><option value="MEDIUM">בינונית</option><option value="LOW">נמוכה</option><option value="UNKNOWN">לא ידועה</option></select></Field><label className="flex items-center gap-2 self-end pb-2 text-sm"><input type="checkbox" checked={values.isVerified === 'true'} onChange={e => set('isVerified', String(e.target.checked))} />אומת מקצועית</label></>}</form>}{open && <div className="mt-3 flex items-center justify-between gap-3">{mutation.isError ? <span className="text-sm text-destructive" role="alert">{errorText(mutation.error)}</span> : <span /> }<Button size="sm" type="submit" form={`feasibility-${path}`} disabled={mutation.isPending}>{mutation.isPending && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}שמירה</Button></div>}</section>
}
