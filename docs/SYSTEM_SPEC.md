# Urban Renewal OS ג€” System Specification Document

**Version:** 2.0 ֲ· **Date:** August 2026 ֲ· **Status:** Authoritative ג€” single source of truth
**Owner:** OpenDoor ׳”׳×׳—׳“׳©׳•׳× ׳¢׳™׳¨׳•׳ ׳™׳× ֲ· **Supersedes:** PRD.md v1, SPEC_V2.md (review draft)

> Conventions used in this document:
> **[RA]** = Recommended Addition (not in original requirements, added for production-grade completeness).
> **[OQ-n]** = Open Question, resolved in ֲ§28.
...
| **Purpose** | Replace Excel sheets, personal WhatsApp, and paper folders with one platform that manages the full urban-renewal lifecycle ג€” lead ג†’ resident organizing ג†’ signatures ג†’ permits ג†’ construction ג†’ delivery. |
...
| **Key value proposition** | The signature threshold (67% ׳׳₪׳×׳—) is computed live from **ownership shares** and drives every dashboard, automation, and decision. One source of truth for the company; self-service transparency for the resident. |

## 2. Product Overview

**What the system does:** Manages urban-renewal projects (׳₪׳™׳ ׳•׳™-׳‘׳™׳ ׳•׳™, ׳×׳"׳ 38) end-to-end: project hierarchy (Project ג†’ Complex ג†’ Building ג†’ Apartment), an ownership registry with fractional shares, resident CRM with a unified interaction timeline, legal signature packages with digital + wet-signature flows, a live threshold engine, multi-channel communications (WhatsApp/SMS/email), tasks/meetings, documents, dashboards, and a resident portal with OTP login.
...
**Main use cases:** UC1 signature campaign management ֲ· UC2 resident onboarding to portal ֲ· UC3 field visit + wet signature capture ֲ· UC4 lawyer package approval ֲ· UC5 stage advancement with checklist ֲ· UC6 tenant onboarding with data import ֲ· UC7 executive KPI review.

**System boundaries ג€” included:** everything above, V1 scope per ֲ§24.
...
- Policy predicates evaluated server-side: `tenant` (always) גˆ§ (`own` | `assigned` | role grant).
...
### 4.2 Projects
Purpose: lifecycle backbone. Users: PM, admin, viewers. Features: 12-stage stepper with **gated advancement** (required-artifact checklist per stage), Complex/Building/Apartment hierarchy, team assignment, stage history. Screens: Projects list, Project detail (tabs: Overview/Buildings/Residents/Signatures/Documents/Timeline/Team/Settings), New Project wizard. Actions: create, advance/revert stage (with reason), assign team. Data: Project, Complex, Building, Apartment, ProjectMember, ProjectStageHistory. Automations: stage-change ג†’ notifications + broadcast draft.

### 4.3 Ownership Registry **[RA ג€” core]**
Purpose: legal source of truth for who must sign. Users: PM, lawyer, admin. Features: owners with fractional shares per apartment (numerator/denominator), multi-apartment owners, estates (׳¢׳™׳–׳‘׳•׳), POA holders, owners abroad; share-sum validation (must equal 1 per apartment; warning badge otherwise). Screens: Apartment drawer (owners table), Owner profile. Data: Owner, OwnerApartment. Dependencies: Import module. Automations: share-sum ג‰  1 ג†’ data-quality task.

### 4.4 Residents & Consent
Purpose: CRM for humans in the project. Features: profile, unified interaction timeline (calls/visits/messages/meetings), signature status, objection management (reason taxonomy: financial/emotional/legal/absentee/dispute; escalation levels), consent registry per channel (timestamped, source-attributed), communication preferences (language, channel priority, quiet hours). Screens: Residents list, Resident profile. Data: Resident, ResidentActivity, Objection, Consent, CommunicationPreference. Automations: OBJECTOR status ג†’ follow-up task 48h; inactive 30d ג†’ re-invite.

### 4.5 Signatures
Purpose: the core loop. Features: SignaturePackage (versioned doc bundle; DRAFTג†’APPROVEDג†’LOCKED by lawyer), per-owner SignatureRecord (DIGITAL via Comsign / WET via field capture with witness+geo, lawyer validation), refusal/revocation flow, **Threshold Engine**: live % per project & building, basis SHARES or UNITS, configurable required % (default 67). Screens: Signatures board (building ֳ— apartment ֳ— owner matrix), Package manager, Threshold gauge (embedded in dashboards). Data: SignaturePackage, SignatureRecord, ThresholdRule. Dependencies: Ownership, Documents, Comsign. Automations: signed ג†’ recompute + milestone events (25/50/67%); refused ג†’ objection record.

### 4.6 Documents
Purpose: vault + generation. Features: upload (drag-drop, 25MB, allowlist: pdf/docx/xlsx/jpg/png), categories, versioning (immutable versions, latest pointer), per-role permissions, expiry dates with reminders **[RA]**, mail-merge generation from templates (resident+apartment placeholders) **[RA]**. Screens: Documents tab, Template manager (V1: upload DOCX with placeholders). Data: Document, DocumentVersion, ResidentDocument. Automations: upload to package ג†’ lawyer notification; expiry-30d ג†’ task.

### 4.7 Communications
Purpose: all resident/staff messaging. Features: WhatsApp Business Cloud (approved templates), SMS (InfoRU, Twilio fallback), email (SES); template manager per language incl. approval status; bulk send with audience builder + preview-as-resident; **outbox pattern** (queue ג†’ provider ג†’ delivery receipt ג†’ status); consent + quiet-hours enforcement (21:00ג€“08:00 queue-delay); fallback WhatsAppג†’SMS once; undelivered report. Data: Message, MessageTemplate, Campaign (V2). Automations: ֲ§10.

### 4.8 Tasks & Meetings
Features: tasks with assignee/due/priority/SLA, meeting scheduling with attendees + outcomes, overdue escalation. Data: Task, Meeting, MeetingAttendee. Automations: overdue 24h ג†’ manager escalation.

### 4.9 Leads & Intake
Features: website form capture, pipeline Kanban, round-robin assignment, first-touch SLA (4 business hours), conversion ג†’ project. Data: Lead, LeadActivity.

### 4.10 Resident Portal
Features: SMS-link invite ג†’ OTP login ג†’ onboarding (confirm details, language, consents) ג†’ stage timeline in plain language, personal document vault, sign-request cards, message thread. Mobile-first, he/ru (V1), ar/en (V2). Data: reuses core entities via portal-scoped API.
...
### 4.12 Dashboards & Reports ג€” ֲ§7, ֲ§14.

### 4.13 Automations ג€” rules engine, ֲ§10.

### 4.14 Admin & Platform
Features: tenant settings (branding, locales), user management, **Excel import wizard** (upload ג†’ column map ג†’ validation report ג†’ dry-run ג†’ commit; ImportBatch with rollback) **[RA]**, feature flags per tenant **[RA]**, audit log viewer, data export (JSON+files ZIP) **[RA]**, manual billing record (V1).

### 4.15 AI Layer (V2+) ג€” ֲ§16.

## 5. User Journeys & Workflows

### W1 ג€” Tenant onboarding (SaaS)
Start: signed contract. User: SUPER_ADMIN + COMPANY_ADMIN. Steps: create tenant ג†’ admin invite email ג†’ branding setup ג†’ template pack cloned (he) ג†’ Excel import (apartments+owners+residents) ג†’ validation report ג†’ fix/commit ג†’ go-live checklist ג†’ 30-day success review. Exceptions: import validation failures (downloadable error file, nothing committed); duplicate tenant slug (blocked).

### W2 ג€” Project intake ג†’ active campaign
Trigger: qualified lead or direct creation. Steps: create project (DISCOVERY) ג†’ hierarchy setup (import or manual) ג†’ threshold rules configured (default 67% SHARES) ג†’ team assigned ג†’ stage RESIDENT_ORGANIZING ג†’ kickoff broadcast (PM-approved) + portal invites. System: baseline signed % captured at import **[RA ג€” enables ROI metric]**. End: campaign live. Edge: missing ownership data ג†’ apartments flagged, count as unsigned.

### W3 ג€” Signature collection (core loop)
Trigger: lawyer approves package (DRAFTג†’APPROVED; LOCKED on first send). Per owner-apartment share, records created PENDING. Digital: send link ג†’ Comsign ג†’ webhook ג†’ SIGNED + certificate stored. Field: agent visit ג†’ wet capture (photo/witness/geo) ג†’ status PENDING_VALIDATION ג†’ lawyer validates ג†’ SIGNED. Every SIGNED ג†’ threshold recompute ג†’ milestone automations. Refusal: REFUSED + Objection(reason) ג†’ follow-up cadence ג†’ escalation to lawyer after 3 attempts. Exceptions: expired link (re-issue), owner deceased (estate flow: mark isEstate, block signature until heirs registered ג€” **[OQ-3]**), revocation window (**[OQ-4]**).

### W4 ג€” Resident portal lifecycle
Invite SMS ג†’ OTP (6-digit, 5min TTL, 5 attempts, 3/hour per phone) ג†’ onboarding ג†’ ongoing use. Exceptions: phone mismatch ג†’ support ticket path; OTP provider down ג†’ fallback provider; no smartphone ג†’ field agent proxy flow (agent shows info, resident signs paper).

### W5 ג€” Stage advancement
PM clicks advance ג†’ checklist evaluated (e.g., PERMITS requires threshold reached + protocol doc) ג†’ if unmet, blocking dialog lists missing artifacts ג†’ if met, stage saved + history row + team notification + optional resident broadcast draft. Revert requires reason (audited).

### W6 ג€” Field visit
Agent opens PWA day-list ג†’ navigates to visit ג†’ completes form (met/not-met, outcome, next step) ג†’ optional signature capture ג†’ offline-safe save ג†’ sync. Exception: sync conflict ג†’ server wins, agent sees diff notice.

## 6. Screen-by-Screen Specification

Global standards applying to **every** screen: RTL-first; skeleton loading; error boundary with retry + support code; toasts on success; server-side pagination (25/page default); typed-confirm modal for destructive actions; breadcrumbs on nested pages; WCAG AA; responsive (ג‰¥1280 full layout, 768ג€“1279 collapsed sidebar, <768 mobile stack).
...
| S2 | Dashboard | /dashboard | staff | ֲ§7 |
| S3 | Projects list | /projects | staff | Table: name, city, stage chip, threshold mini-bar, units, PM; filters: stage/city/PM; search; New Project. Empty: "׳¦׳•׳¨ ׳₪׳¨׳•׳™׳§׳˜ ׳¨׳׳©׳•׳" CTA |
| S4 | Project detail | /projects/:id | assigned | Header: stage stepper + threshold gauge; 8 tabs (ֲ§4.2). Advance-stage button gated per W5 |
| S5 | New Project wizard | /projects/new | PM+ | 4 steps: details ג†’ hierarchy (manual/import) ג†’ threshold rules ג†’ team. Validation per step; draft saved between steps |
| S6 | Apartment drawer | (overlay) | assigned | Owners+shares table (sum badge), signature status per owner, docs, notes. Inline add-owner with share fraction inputs (validate denominator>0, sumג‰₪1) |
...
| S9 | Signatures board | /signatures | PM, RR, lawyer | Matrix buildingֳ—apartmentֳ—owner; status chips; filters; bulk send; refusal heatmap toggle; package selector |
...
| S13 | Communications | /communications | RR, PM | Template manager (language tabs, WhatsApp approval badge), bulk composer (audience builder ג†’ preview-as-resident ג†’ schedule/send), delivery report |
| S14 | Import wizard | /admin/import | admin, PM | Upload ג†’ column mapping (auto-detect Hebrew headers) ג†’ validation report table (row, field, error) + downloadable errors ג†’ dry-run summary ("׳™׳•׳•׳¦׳¨׳•: 120 ׳“׳™׳¨׳•׳×, 214 ׳‘׳¢׳׳™׳") ג†’ commit. All-or-nothing per batch |
| S15 | Reports | /reports | CEO, admin, PM | ֲ§14 |
| S16 | Admin settings | /admin/* | admin | Users, branding, flags, audit viewer, export |
| P1 | Portal home | portal:/ | resident | Stage timeline (plain language), threshold NOT shown (**[OQ-5]** ג€” recommended: show project-level % only after PM opt-in) |
| P2 | Portal documents | portal:/documents | resident | Personal vault + shared project docs |
| P3 | Portal sign | portal:/sign/:id | resident | Package summary, doc preview, big CTA ג†’ Comsign redirect; status after |
...
- Per-project threshold progress bars (target line at required %), click ג†’ project.
...
|---|---|---|---|
| Tenant | SaaS customer | name (str, req), slug (unique), settings (json) | 1ג€”N all |
| User | Staff account | email (unique/tenant, email format), passwordHash (scrypt), role (enum 13), isActive, mfaSecret? | Nג€”1 Tenant; sessions |
| Session | Refresh tokens | userId, tokenHash, expiresAt, revokedAt? | index (userId) |
| Project | Lifecycle root | code (unique/tenant), name req, city req, stage (enum 12), status, signatureGoal (int, default 67), totalUnits/signedUnits (derived cache) | 1ג€”N Complex, ProjectMember, StageHistory |
...
| **Owner** | Legal person | fullName req; nationalId? (encrypted, ׳×"׳– checksum validated); isEstate; residentId? unique | Nג€”M Apartment via OwnerApartment |
| **OwnerApartment** | Share row | shareNumerator/Denominator (int >0); unique (ownerId, apartmentId) | per-apartment sum ג‰₪ 1 (app-enforced, warning if ג‰ 1) |
| Resident | CRM profile | names req; phone (IL format E.164); language; signatureStatus enum; doNotContact | Nג€”1 Apartment; timeline children |
| **Consent** [RA] | Channel consent | residentId, channel enum, granted bool, source, timestamp | append-only |
| **Objection** [RA] | Refusal tracking | reason enum(5), escalationLevel int, resolvedAt? | Nג€”1 Resident |
| **SignaturePackage** | Doc bundle | version int, status DRAFT/APPROVED/LOCKED, documentIds json, approvedById | 1ג€”N SignatureRecord; immutable after LOCKED |
...
| **ThresholdRule** | Majority config | scope PROJECT/BUILDING, requiredPct float (0ג€“100], basis SHARES/UNITS | unique (projectId, scope, buildingId) |
| Document / DocumentVersion | Vault | category enum, fileName, fileSize ג‰₪ 25MB, mime allowlist, expiresAt? | versions immutable |
...
**ERD (core):** Tenant ג¶ Project ג¶ Complex ג¶ Building ג¶ Apartment גµ OwnerApartment ג¶ Owner; Apartment גµ Resident; SignaturePackage ג¶ SignatureRecord גµ Owner; Project גµ ThresholdRule; Resident גµ {Consent, Objection, Activity, Message}.

## 9. Business Rules

1. **Threshold computation:** signedPct = ־£(signed share fractions per apartment, capped 1.0) / apartment count ֳ— 100 (basis SHARES); basis UNITS counts apartment only when all shares signed. No ownership rows ג†’ apartment counts fully unsigned. Recompute on every SignatureRecord transition; cached on Project (signedUnits), never trusted for legal display (always recomputed for the gauge).
...
3. **Status transitions:** SignatureRecord PENDINGג†’SIGNED|REFUSED; SIGNEDג†’REVOKED (lawyer only, reason req); REFUSEDג†’PENDING (re-engage). Package DRAFTג†’APPROVED (lawyer)ג†’LOCKED (first send); LOCKED is terminal except new version (copies docs, increments version).
4. **Validation:** Israeli ID checksum (biometric algorithm); phone normalized to E.164 (+972); share denominators > 0; share sum warning ג‰  1, hard error > 1; gush/helka numeric formats.
5. **Consent rule:** no resident-facing send without granted consent for that channel; quiet hours 21:00ג€“08:00 delay queue; ׳—׳•׳§ ׳”׳¡׳₪׳׳ compliance ג€” every marketing-class message carries opt-out.
...
8. **Exception handling:** integration failure ג†’ outbox retry ֳ—3 exponential ג†’ mark FAILED ג†’ surface in undelivered report + admin alert.

## 10. Automations (V1 rule set ג€” config-as-JSON, BullMQ worker)

| # | Trigger | Condition | Action | Recipient | Timing | On failure | Log |
|---|---|---|---|---|---|---|---|
| A1 | signature.signed | ג€” | recompute threshold; notify PM | PM | immediate | retry ֳ—3 | AuditLog + job log |
| A2 | threshold.milestone (25/50/67) | first crossing | create broadcast draft + celebration task | PM | immediate | retry | yes |
| A3 | resident.status=OBJECTOR | ג€” | follow-up task | RR manager | +48h | retry | yes |
| A4 | task.overdue | 24h past due | escalation notification | assignee's manager | daily 08:00 | skip+log | yes |
| A5 | lead.created | ג€” | round-robin assign + first-touch task (SLA 4bh) | assignee | immediate | fallback: unassigned queue | yes |
...
| A8 | project.stage.changed | ג€” | team notification + resident broadcast **draft** | team, PM | immediate | retry | yes |
| A9 | document.expiresAt | גˆ’30d | renewal task | doc owner | daily 08:00 | skip+log | yes |

All automations idempotent (dedupe key = trigger+entity+rule), failures visible in admin log viewer.

## 11. Notifications & Communication

Channels: in-app (bell + feed, mark-read), email (SES ג€” internal digests, portal fallback), SMS (OTP + critical), WhatsApp (resident default, approved templates only), PWA push (agents, V1-optional).
**Preference matrix:** per user & per resident: event-category ֳ— channel toggle; defaults: staff = in-app+daily email digest; residents = WhatsAppג†’SMS fallback. Quiet hours enforced for resident sends.
...
**Trigger conditions:** every automation in ֲ§10 + manual sends. Delivery statuses tracked per message; digest batches internal notifications (default on).

## 12. Document Management

Types (category enum): contract, POA, tabu extract, protocol, permit, plan, correspondence, id-copy, other.
Upload: drag-drop, ג‰₪25MB, mime allowlist, virus scan **[RA ג€” V1: extension+magic-byte check; V2: ClamAV]**. Download: signed URLs (15-min expiry).
Versioning: immutable versions, latest pointer, "restore as new version."
Permissions: per-category role map (legal docs ג†’ lawyer/PM/admin; resident sees only own + published project docs).
...
Global search (topbar, Ctrl+K): residents, owners, projects, apartments by name/phone/ID-last-4/address ג€” Postgres FTS + pg_trgm (Hebrew-adequate at this scale; Elasticsearch deferred).
Module filters: enumerated per screen (ֲ§6); **saved filters** per user; sort on all table columns (server-side).
Bulk actions: residents (message, task, export, tag), signatures (send), tasks (reassign). Bulk bar shows count + undo where possible.
Export: filtered views ג†’ XLSX/CSV; full tenant export ג†’ ZIP (admin).
Import: ֲ§4.14 wizard; batches auditable + rollback-able.
Pagination: cursor-based, 25 default, 100 max.

## 14. Reports & Analytics

Standard V1 reports (server-rendered + XLSX): signature progress & weekly velocity per project; ownership coverage gaps (apartments with share-sumג‰ 1, unreachable owners); field-agent activity; communication delivery; task SLA; portal adoption; **baseline vs current signed %** (ROI proof).
...
|---|---|---|---|---|---|---|
| Comsign | Qualified digital signature | package docs out; signed PDF + cert in | API key (vault) | signing status | retry ֳ—3 ג†’ manual task | vendor limit, queue |
...
| Google Maps | Geocoding, map pins | addressג†’latlng | API key | ג€” | cache results; degrade to text | budget alert |
| Comsign/Textract OCR, Tabu vendor, iCount | V2 | ג€” | ג€” | ג€” | ג€” | ג€” |
...
|---|---|---|---|---|---|---|
| Meeting summary | PM/RR | meeting audio/notes | Claude summarization | summary + action items ג†’ tasks draft | PM approves tasks | no resident PII to model beyond names; DPA with provider |
...
| Signature-likelihood scoring | RR | interaction history | classification | priority ranking | advisory only ג€” never auto-messages | explainable factors shown |
| Message draft assist | RR | context + intent | generation | draft in resident's language | always human-sent | template-constrained |

Failure handling: AI unavailable ג†’ feature hidden, no degradation of core flows.

## 17. Security

- **AuthN:** scrypt (N=2^15) password hashing with transparent upgrade-on-login from legacy; password policy ג‰¥10 chars + breach-list check **[RA]**; TOTP MFA mandatory COMPANY_ADMIN+, optional others; OTP per ֲ§5-W4 limits.
- **AuthZ:** RBAC per ֲ§3; server-side policy checks on every route; portal scoped by OwnerApartment.
...
- **Audit:** ֲ§22. **Backup/recovery:** Postgres PITR, daily snapshots 30d, quarterly restore drill, RPO ג‰₪1h, RTO ג‰₪4h.
- **Privacy compliance:** Israeli Privacy Protection Law incl. Amendment 13 ג€” DPO appointed, records of processing, 72h breach-notification runbook, data-minimization review, resident data-subject requests (export/correct) via support flow; GDPR-aligned.
...
**Decision: modular monolith** (NestJS modules = future service seams), not the 12-microservice design of V1 ג€” one deploy, transactions across modules, matches team capacity. Extraction trigger: sustained load or team ג‰¥3 backend devs per domain.

| Layer | Choice |
|---|---|
| Frontend | Next.js 15 App Router ֳ—3 (crm/portal/web) on Vercel; React Query; next-intl RTL |
| Backend | NestJS single API service + BullMQ worker process, on Railway/Render (Docker) |
| DB | Managed PostgreSQL, PITR; Prisma **pinned v6**; PostgreSQL for local dev |
...
| CI/CD | GitHub Actions: lint ג†’ typecheck ג†’ unit ג†’ integration (Docker PG) ג†’ migrate-deploy gate ג†’ deploy; preview deploys per PR (Vercel) |
| Environments | local (PostgreSQL) ג†’ staging (full stack, seeded) ג†’ production |
...
Components (ShadCN-based): buttons (primary/secondary/ghost/destructive; loading state built-in), forms (label-above, inline validation on blur, error text below field, RTL-correct number inputs), tables (sticky header, sortable, row hover, selection column), modals (max 560px, typed-confirm for destructive), drawers for detail-in-context, toasts (sonner, bottom-left in RTL), status chips (stage/signature/consent color-coded with icons ג€” never color-only), progress/threshold gauge (target line marker), timeline component, empty states (icon + one CTA), skeletons per layout.
Accessibility: WCAG AA ג€” contrast ג‰¥4.5:1, visible focus rings, full keyboard nav, aria labels, form errors announced; icons mirrored correctly in RTL (chevrons, steppers).
Responsive: breakpoints per ֲ§6 global standards; tables collapse to cards <768px.
...
|---|---|
| Comsign down | outbox retry ֳ—3 ג†’ record stays PENDING, task to PM, admin alert |
...
**V2 (months 5ג€“9):** visual automation builder (rule engine already underneath); campaigns; ticketing; OCR (Textract); AI meeting summaries + message assist; Arabic+English; public API + webhooks; Metabase BI; Tabu integration; billing automation; ClamAV. *Postponed because each depends on V1 data volume/feedback and none blocks the core consent pipeline.*
**V3 (10ג€“16):** native apps (offline depth beyond PWA), AI chatbot + scoring, voting with quorum rules (needs legal spec), municipality portal, multi-country, SOC2.
...
Testing: unit (services, threshold engine golden cases incl. fractions/estates), integration (API + real PG in CI), permission tests asserting cross-tenant denial per endpoint, E2E happy paths (Playwright: login, create project, import, sign flow); coverage gate 70% V1 ג†’ 80%.
Security: no secrets in repo; dependency scanning (Renovate); ֲ§17 items are release-blocking.
Documentation: OpenAPI (Swagger already wired) kept accurate; ADRs for architecture decisions; runbooks (deploy, restore, breach).
Deployment: migrations via `prisma migrate deploy` gate before app start; blue-green or health-checked rollout; rollback = previous image + migration policy (expand-contract only ג€” no destructive migrations in same release as code).
...
- **Ownership:** import of 3 owners ֳ— fractional shares sums correctly; share-sum >1 blocked; apartment with no owners shows flagged and counts unsigned.
- **Threshold:** golden test set passes ג€” e.g., 10 apartments, one with 2 owners ֲ½+ֲ½ where one signed ג†’ project 5.0% (SHARES) / 0 units (UNITS at that apartment); milestone automation fires exactly once per crossing.
- **Signatures:** digital flow round-trips Comsign sandbox ג†’ record SIGNED with evidence doc; wet flow requires lawyer validation before counting; LOCKED package rejects doc changes.
...
| OQ-2 | Which majority rules per project type (67% ׳₪׳™׳ ׳•׳™-׳‘׳™׳ ׳•׳™ vs ׳×׳"׳ variants), per-building AND/OR project | Default 67% SHARES project-scope; configurable per project; confirm with counsel | Needs confirmation |
| OQ-3 | Estate (׳¢׳™׳–׳‘׳•׳) handling: who may sign before probate | Block signature; allow POA-holder flow only with lawyer override (audited) | **Legal decision needed** |
